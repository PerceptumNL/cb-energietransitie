...
function create(proto,trait){var self=Object_create(proto);var properties={};forEach(getOwnPropertyNames(trait),function(name){var pd=trait[name];if(pd.required){if(!(name in proto)){throw new Error('Missing required property: '+name);}}else if(pd.conflict){throw new Error('Remaining conflicting property: '+name);}else if('value'in pd){if(pd.method){properties[name]={value:freezeAndBind(pd.value,self),enumerable:pd.enumerable,configurable:pd.configurable,writable:pd.writable};}else{properties[name]=pd;}}else{properties[name]={get:pd.get?freezeAndBind(pd.get,self):undefined,set:pd.set?freezeAndBind(pd.set,self):undefined,enumerable:pd.enumerable,configurable:pd.configurable};}});defineProperties(self,properties);return freeze(self);}
...
Trait.required=freeze(required);Trait.compose=freeze(compose);Trait.resolve=freeze(resolve);Trait.override=freeze(override);Trait.create=freeze(create);Trait.eqv=freeze(eqv);Trait.object=freeze(object);return freeze(Trait);})();if(typeof exports!=="undefined"){exports.Trait=Trait;}function shuffle(array) {
...
var Questionnaire = {
  ...
  drawNumbers: function() {
    ...
    for (var i=0; i<nquestions; i++) {
      ...
      if (i<nquestions-1) {
        $("#number").append("<span class='number-sep'>*</span>")  
      }
    }
